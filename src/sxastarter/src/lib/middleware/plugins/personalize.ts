...
class PersonalizePlugin implements MiddlewarePlugin {
  private personalizeMiddleware: PersonalizeMiddleware | null = null;

  // Using 1 to leave room for things like redirects to occur first
  order = 1;

  private getPersonalizeMiddleware(): PersonalizeMiddleware | null {
    ...
    return this.personalizeMiddleware;
  }

  async exec(req: NextRequest, res?: NextResponse): Promise<NextResponse> {
    const middleware = this.getPersonalizeMiddleware();
    if (!middleware) {
